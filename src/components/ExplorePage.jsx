...
const CourseCategories = () => {
  ...
  const fetchCategories = async () => {
    try {
      const response = await fetch("http://92.205.62.64:5254/api/Lookup/GetCourseCategories");
      ...
      <div className="max-w-6xl mx-auto text-center py-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          {categories && categories.map((category, index) => (
            ...
            >
              <img className="text-2xl mr-3" src={category.icon?category.icon:icon1}/>
              <span className="font-medium text-gray-700">{category.title}</span>
            </div>
          ))}
        </div>
        <button className="px-6 pr-8 py-3 bg-[#074226] text-white font-medium rounded hover:bg-green-700 transition relative" onClick={()=>{navigate('/discover')}}>
        {t("exploreMore")}
